...
if($row1>0 || $row2>0){
		$result1 = $db->insert("DELETE FROM laboratory WHERE username='$username'");
		$result2 = $db->insert("DELETE FROM user WHERE user_name='$username'");
		echo '<script type="text/javascript">alert("Laboratory was deleted!");history.go(-1);</script>';
}
else{
	echo '<script type="text/javascript">alert("No such a Laboratory...");history.go(-1);</script>';
}
