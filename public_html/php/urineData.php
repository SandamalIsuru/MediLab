...
$result = $db->query("SELECT * FROM expire WHERE username='$PId'");
$row = mysqli_fetch_assoc($result);
if($row>0){
	echo "hi";
	$y = $db->insert("UPDATE expire SET latest_date='$date' WHERE username='$PId'");
}else{
	$y = $db->insert("INSERT INTO expire VALUES ('$PId','$date')");
}
if($x == 1){
	$user = "94717307845";
	$password = "1911";
